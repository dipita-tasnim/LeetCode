...
# s1 = "ab" (length 2)  →  window is ALWAYS size 2
# s2 = e i d b a o o o
#      [_]                 window at position 0
#        [_]               slide right by 1
#          [_]             slide again...


def checkInclusion(s1, s2):
    if len(s1) > len(s2):        # ← ADD THIS
        return False
    # count characters in s1 (target)
    need = {}
    for i in s1:
        if i in need:
            need[i] += 1
    ...
    # count characters in the first window (every window len = s1 len) of s2
    window = {}
    ...
    if window == need:   # first window alreday matches
        return True
    ...
    for i in range(len(s1), len(s2)):
        ...
        if window[left_char] == 0:
            del window[left_char] # remove zero counts so that == comparison works [both key, val deleted]   
